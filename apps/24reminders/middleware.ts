...
export async function middleware(req: NextRequest) {
  ...
  const publicUrl = `${req.headers.get('x-forwarded-proto') || 'https'}://${req.headers.get('x-forwarded-host') || req.headers.get('host') || 'reminders.relentify.com'}${req.nextUrl.pathname}${req.nextUrl.search}`
