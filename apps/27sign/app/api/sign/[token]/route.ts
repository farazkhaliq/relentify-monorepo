...
export async function GET(req: NextRequest, { params }: { params: Promise<{ token: string }> }) {
  ...
  if (sr.status === 'expired' || (sr.expires_at && new Date(sr.expires_at) < new Date())) {
    if (sr.status !== 'expired') {
      await query("UPDATE signing_requests SET status = 'expired' WHERE id = $1", [sr.id])
    }
  ...
  const { rows: otpRows } = await query(
    'SELECT verified_at FROM otp_codes WHERE signing_request_id = $1 ORDER BY created_at DESC LIMIT 1',
    [sr.id]
  )
