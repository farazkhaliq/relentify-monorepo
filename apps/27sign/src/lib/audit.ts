...
export async function appendAuditLog(entry: AuditEntry): Promise<void> {
  ...
  const { rows: prev } = await query(
    'SELECT entry_hash FROM audit_log WHERE signing_request_id = $1 ORDER BY created_at DESC LIMIT 1',
  ...
  await query(
    `INSERT INTO audit_log (signing_request_id, action, ip, user_agent, details, entry_hash, previous_hash, created_at)
