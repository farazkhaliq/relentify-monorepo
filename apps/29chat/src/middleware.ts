...
export async function middleware(req: NextRequest) {
  ...
  // Allow static, API, widget, portal, and asset paths
  if (
  ...
  // Protected app routes — check JWT, redirect to auth.relentify.com
  const token = req.cookies.get(AUTH_COOKIE_NAME)?.value
  const publicUrl = `${req.headers.get('x-forwarded-proto') || 'https'}://${req.headers.get('x-forwarded-host') || req.headers.get('host') || 'chat.relentify.com'}${req.nextUrl.pathname}${req.nextUrl.search}`
