...
import { getExpenseApprovalSettings, rejectMileage } from '@/src/lib/expense_approval.service';
...
export async function POST(
  ...
  try {
    ...
    if (!approvalSettings?.enabled || approvalSettings.approver_user_id !== auth.userId) {
      return NextResponse.json({ error: 'Not authorised to reject mileage claims' }, { status: 403 });
    }
    ...
    const ok = await rejectMileage(id, auth.userId, reason.trim());
    if (!ok) {
    ...
    console.error('POST mileage/reject error:', e);
