...
export async function createIntercompanyTransaction(
  ...
  const invRes = await query(
    `SELECT * FROM acc_invoices WHERE id = $1 AND entity_id = $2 AND user_id = $3`,
  ...
  return withTransaction(async (client) => {
    ...
    const billRes = await client.query(
      `INSERT INTO acc_bills (user_id, entity_id, supplier_name, amount, vat_rate, vat_amount, currency, due_date, category, notes, reference)
    ...
    const linkRes = await client.query(
      `INSERT INTO acc_intercompany_links (initiating_entity_id, receiving_entity_id, source_invoice_id, mirror_bill_id, amount, currency)
...
export async function getIntercompanyLinks(entityId: string): Promise<Record<string, unknown>[]> {
  const r = await query(
    `SELECT il.*,
     ...
     LEFT JOIN acc_invoices i ON i.id = il.source_invoice_id
