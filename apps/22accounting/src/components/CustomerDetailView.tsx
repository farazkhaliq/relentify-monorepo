...
export default function CustomerDetailView({ customer, invoices, stats }: { customer: any; invoices: any[]; stats: any }) {
  ...
  return (
    <div className="space-y-6 sm:space-y-8">
      ...
      {confirmDelete && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-[var(--theme-primary)]/60 backdrop-blur-sm">
          <Card variant="default" padding="lg" className="relative w-full max-w-md shadow-2xl space-y-6">
            <div>
              <h3 className="text-base font-black text-[var(--theme-text)] mb-2">Delete {customer.name}?</h3>
      ...
      <PageHeader
        title={customer.name}
        description={
          <Link href="/dashboard/customers" className="text-[var(--theme-text-muted)] hover:text-[var(--theme-text)] text-xs font-bold no-underline">
            ← Back to Customers
          </Link>
        }
        actions={
          <div className="flex gap-2 sm:gap-3">
            {!editing ? (
                ...
                {hasInvoices ? (
                  <div className="relative group">
                    ...
                    <div className="absolute bottom-full right-0 mb-2 w-56 bg-[var(--theme-card)] border border-[var(--theme-border)] rounded-cinematic px-3 py-2 text-[10px] text-[var(--theme-text-muted)] hidden group-hover:block shadow-xl z-10 uppercase font-black tracking-wider">
                      Can't delete — {invoices.length} invoice{invoices.length !== 1 ? 's' : ''} on record.
                    </div>
                  </div>
                ) : (
              ...
              <>
                <Button onClick={() => { setEditing(false); setForm({ name: customer.name, email: customer.email, phone: customer.phone, address: customer.address, notes: customer.notes }); }} variant="ghost" className="bg-[var(--theme-border)]/50 border border-[var(--theme-border)] rounded-cinematic text-xs sm:text-sm uppercase tracking-widest font-black">
                  Cancel
                </Button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
        <Card variant="default" padding="md" className="flex flex-col items-center justify-center text-center">
          <p className="text-[10px] font-black text-[var(--theme-text-muted)] uppercase tracking-widest mb-2">Invoiced</p>
          <p className="text-2xl sm:text-3xl font-black text-[var(--theme-text)]">£{stats.totalInvoiced.toFixed(2)}</p>
        </Card>
        <Card variant="default" padding="md" className="flex flex-col items-center justify-center text-center">
      ...
      <Card variant="default" padding="lg" className="space-y-6">
        <h3 className="text-[10px] font-black text-[var(--theme-accent)] uppercase tracking-widest mb-4">Customer Details</h3>
        {editing ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div className="space-y-1">
              <Label>Email</Label>
              <p className="text-[var(--theme-text)] text-sm font-medium">{customer.email || '—'}</p>
            </div>
            <div className="space-y-1">
              <Label>Phone</Label>
              <p className="text-[var(--theme-text)] text-sm font-medium">{customer.phone || '—'}</p>
            </div>
            <div className="sm:col-span-2 space-y-1">
              <Label>Address</Label>
              <p className="text-[var(--theme-text)] whitespace-pre-wrap text-sm font-medium leading-relaxed">{customer.address || '—'}</p>
            </div>
            {customer.notes && (
              <div className="sm:col-span-2 space-y-1">
                <Label>Notes</Label>
                <p className="text-[var(--theme-text-muted)] whitespace-pre-wrap text-sm font-medium leading-relaxed italic">{customer.notes}</p>
              </div>
            )}
          </div>
        )}
      ...
      <Card padding="none" className="overflow-hidden">
        <div className="p-4 sm:p-6 border-b border-[var(--theme-border)] flex items-center justify-between">
          <h3 className="text-[10px] font-black text-[var(--theme-accent)] uppercase tracking-widest">Invoices ({invoices.length})</h3>
          <Link href="/dashboard/invoices/new" className="text-[10px] font-black text-[var(--theme-accent)] uppercase tracking-widest hover:brightness-110 no-underline">+ New</Link>
        </div>

        {invoices.length === 0 ? (
          <div className="p-12 sm:p-16 text-center">
            <p className="text-[var(--theme-text-muted)] font-medium mb-4">No invoices yet</p>
          ...
          <>
            ...
            <div className="md:hidden divide-y divide-[var(--theme-border)]">
              {invoices.map(inv => (
                <div key={inv.id} onClick={() => router.push(`/dashboard/invoices/${inv.id}`)} className="p-4 hover:bg-[var(--theme-border)]/30 transition-colors cursor-pointer">
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="text-[var(--theme-text)] font-black text-sm">{inv.invoice_number}</span>
                    <Badge variant={SC[inv.status] || 'default'}>{inv.status}</Badge>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-[var(--theme-text-dim)] text-xs">{new Date(inv.issue_date).toLocaleDateString('en-GB')}</span>
                    <span className="text-[var(--theme-text)] font-bold text-sm">£{parseFloat(inv.total).toFixed(2)}</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            <Table className="hidden md:table">
              ...
              <TableBody>
                {invoices.map(inv => (
                  <TableRow key={inv.id} onClick={() => router.push(`/dashboard/invoices/${inv.id}`)} className="cursor-pointer">
                    <TableCell className="text-[var(--theme-text)] font-black">{inv.invoice_number}</TableCell>
                    <TableCell className="text-[var(--theme-text-muted)]">{new Date(inv.issue_date).toLocaleDateString('en-GB')}</TableCell>
                    <TableCell>
                      <Badge variant={SC[inv.status] || 'default'}>{inv.status}</Badge>
                    </TableCell>
                    <TableCell className="text-[var(--theme-text)] font-bold text-right">£{parseFloat(inv.total).toFixed(2)}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
