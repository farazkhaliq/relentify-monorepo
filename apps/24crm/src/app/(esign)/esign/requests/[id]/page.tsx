...
export default async function RequestDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { rows: reqRows } = await query(
    `SELECT sr.*, s.image_data as signature_image
     ...
     LEFT JOIN esign_signatures s ON sr.signature_id = s.id
  ...
  if (req.document_id) {
    const { rows: docRows } = await query(
      'SELECT id, original_filename, page_count FROM esign_documents WHERE id = $1',
      [req.document_id]
    )
  ...
  const { rows: auditRows } = await query(
    'SELECT * FROM esign_audit_log WHERE signing_request_id = $1 ORDER BY created_at ASC',
    [id]
  )
