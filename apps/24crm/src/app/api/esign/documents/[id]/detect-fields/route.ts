...
export async function POST(
  ...
  const { rows } = await query(
    `SELECT d.id, d.pdf_data, d.page_count
     ...
     JOIN esign_signing_requests sr ON d.signing_request_id = sr.id
     WHERE d.id = $1 AND (sr.created_by_user_id = $2 OR d.signing_request_id IN (SELECT id FROM esign_signing_requests WHERE created_by_user_id = $2))
     LIMIT 1`,
    [id, user.userId]
  )
