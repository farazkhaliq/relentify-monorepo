...
export async function POST(req: NextRequest, { params }: { params: Promise<{ token: string }> }) {
  ...
  if (session) {
    ...
    if (signer) {
      ...
      const { rows: srRows } = await query(
        'SELECT id, signer_email FROM esign_signing_requests WHERE token = $1',
        [token]
      )
  ...
  const { rows: fieldRows } = await query(
    'SELECT id, signer_email, field_type, filled_at FROM esign_document_fields WHERE id = $1',
    [fieldId]
  )
  ...
  const { rows: signerRows } = await query(
    `SELECT srs.status, srs.sign_order, sr.signing_mode
     FROM esign_signing_request_signers srs
     JOIN esign_signing_requests sr ON sr.id = srs.signing_request_id
  ...
  // Update field value
  await query(
    'UPDATE esign_document_fields SET value = $1, filled_at = NOW() WHERE id = $2',
    [value, fieldId]
  )
