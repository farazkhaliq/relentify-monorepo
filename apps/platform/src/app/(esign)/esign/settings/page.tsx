...
export default function SettingsPage() {
  ...
  useEffect(() => {
    fetch('/api/esign/settings/keys').then(r => r.json()).then(d => setKeys(d.keys || []))
    fetch('/api/esign/settings/subscription').then(r => r.json()).then(setSub)
  }, [])

  async function createKey() {
    setCreating(true)
    const res = await fetch('/api/esign/settings/keys', {
      method: 'POST',
  ...
  async function upgrade(tier: string) {
    const res = await fetch('/api/esign/stripe/checkout', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
