...
import { getAuthUser } from '@/lib/services/esign/auth'
...
export default async function DashboardPage() {
  ...
  const { rows } = await query(
    `SELECT sr.id, sr.token, sr.title, sr.signer_email, sr.signer_name,
            ...
            (SELECT COUNT(*) FROM esign_signing_request_signers srs WHERE srs.signing_request_id = sr.id) AS signer_count,
            (SELECT COUNT(*) FILTER (WHERE srs.status = 'signed') FROM esign_signing_request_signers srs WHERE srs.signing_request_id = sr.id) AS signed_count
     FROM esign_signing_requests sr
        ...
        <Card className="border-[var(--theme-accent)]/30 bg-[var(--theme-accent)]/5">
          <CardContent className="p-6 sm:p-8">
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6">
              ...
              <Link href="/esign/getting-started" className="no-underline shrink-0">
      ...
      <Card className="overflow-hidden">
        <CardHeader className="border-b border-[var(--theme-border)] px-4 sm:px-8 md:px-12">
          <div className="flex items-center justify-between">
            ...
            <Link href="/esign/requests/new" className="no-underline">
              <Button variant="primary" className="text-xs">
                + New Request
              </Button>
            </Link>
