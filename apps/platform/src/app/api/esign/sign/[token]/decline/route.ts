...
import { verifySignerSession } from '@/lib/services/esign/signer-session'
import { isOtpVerified } from '@/lib/services/esign/otp'
import { getSignerByToken, markSignerDeclined } from '@/lib/services/esign/signers'
...
import { signerDeclinedEmail } from '@/lib/services/esign/email-templates'
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ token: string }> }) {
  ...
  if (session) {
    ...
    if (signer) {
      ...
      const { rows: srRows } = await query(
        'SELECT id FROM esign_signing_requests WHERE token = $1',
        [token]
      )
  ...
  if (signerId === 'request-level') {
    ...
    await query("UPDATE esign_signing_requests SET status = 'cancelled' WHERE id = $1", [signingRequestId])
  } else {
  ...
  // Notify the sender
  const { rows: srRows } = await query(
    'SELECT sender_email, title, signer_email FROM esign_signing_requests WHERE id = $1',
    [signingRequestId]
  )
