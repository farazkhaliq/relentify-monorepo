...
export async function getOrCreateSubscription(userId: string): Promise<UserSubscription> {
  const { rows } = await query(
    'SELECT * FROM esign_user_subscriptions WHERE user_id = $1',
    [userId]
  )
  ...
  const { rows: created } = await query(
    `INSERT INTO esign_user_subscriptions (user_id, tier, subscription_status)
...
export async function updateSubscription(
  ...
  await query(
    `UPDATE esign_user_subscriptions SET
...
export async function incrementRequestCount(userId: string): Promise<number> {
  const { rows } = await query(
    `UPDATE esign_user_subscriptions
