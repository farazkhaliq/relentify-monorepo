...
export interface ConnectMessage {
  id: string; conversation_id: string; entity_id: string; channel: string
  sender_type: string; sender_id: string | null; body: string
  attachment_url: string | null; external_message_id: string | null
  metadata: Record<string, any>; created_at: string
}

export function createConnectMessageService(pool: Pool) {
  return {
    ...
    async getMessages(conversationId: string, since?: string): Promise<ConnectMessage[]> {
      if (since) {
        const result = await pool.query('SELECT * FROM connect_messages WHERE conversation_id = $1 AND created_at > $2 ORDER BY created_at ASC', [conversationId, since])
        return result.rows
      }
      const result = await pool.query('SELECT * FROM connect_messages WHERE conversation_id = $1 ORDER BY created_at ASC', [conversationId])
      return result.rows
    },
  }
}
